...
function ProjectSection({ title, description, limit }) {
   ...
   const displayedProjects = limit
    ...
    : projectsData;
  return (
    <section className="p-8 md:p-16 lg:p-20 bg-slate-100 flex flex-col gap-12" data-aos="fade-up">
      <div className=" flex flex-col md:flex-row space-y-8 md:space-y-0 p-2 justify-between mb-6" data-aos="fade-up">
        <div>
          <h2 className="text-xl font-bold" data-aos="fade-up">{title}</h2>
          <p className="text-gray-600" data-aos="fade-up">{description}</p>
        </div>
        <div>
          <button className="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition-colors duration-300" data-aos="fade-up">
